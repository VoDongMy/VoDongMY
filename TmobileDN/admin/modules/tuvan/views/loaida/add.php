...
<div class="lang" id="language<?=$lang->lang_id?>" style="display: <?=($lang->lang_default == 1)?'block':'none'?>;">
    <table class="form">
        <tr>
            ...
            <td><input type="text" name="vdata[tenda][<?=$lang->lang_id?>]" value="<?php echo set_value('vdata[tenda]['.$lang->lang_id.']')?>" class="w500"></td>
        </tr>
        <tr>
            <td class="label">Miêu tả</td>
            <td>
                <?=vnit_editor(htmlspecialchars_decode(set_value('vdata[noidung]['.$lang->lang_id.']')),'vdata[noidung]['.$lang->lang_id.']','congdung_'.$lang->lang_id)?>
            </td>
        </tr>
    </table>
</div>
