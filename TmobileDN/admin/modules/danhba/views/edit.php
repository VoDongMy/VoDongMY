        ...
        <td valign="top" style="padding-right: 10px;">
            <table class="form">
                <tr>
                    ...
                    <td>
                        <select name="vdata[cat_id]" style="font-size: 12px;" class="w300">
                            <?foreach($listcategory as $val):
                            $list1 = $this->news->get_all_category($val->cat_id);
                            ?>
                            <option value="<?php echo $val->cat_id?>" <?=($val->cat_id == $rs->cat_id)?'selected="selected"':''?>><?=$val->cat_name?></option>
                                <?foreach($list1 as $val1):
                                $list2 = $this->news->get_all_category($val1->cat_id);
                                ?>
                                    <option value="<?php echo $val1->cat_id?>" <?=($val1->cat_id == $rs->cat_id)?'selected="selected"':''?>>|____|<?=$val1->cat_name?></option>
                                <?endforeach;?>
                            <?endforeach;?>
                        </select>
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="noibat" <?=($rs->option == 1)?'checked="checked"':''?> value="1">Có
                    </td>
                </tr>
                <tr>
        ...
        <div class="lang" id="language<?=$lang->lang_id?>" style="display: <?=($lang->lang_default == 1)?'block':'none'?>;">
            <table class="form">
                <tr>
                    ...
                    <td><input type="text" name="vdata[title][<?=$lang->lang_id?>]" value="<?php echo $lang->title?>" style="width: 98%;"></td>
                </tr>
                <tr>
                    <td colspan="2"><textarea style="width:99%;" rows="5" name="vdata[introtext][<?=$lang->lang_id?>]"><?php echo $lang->introtext?></textarea></td>
                </tr>
                <tr>
                    <td colspan="2"><?=vnit_editor($lang->fulltext,'vdata[fulltext]['.$lang->lang_id.']','full_'.$lang->lang_id)?></td>
                </tr>
            </table>
        </div>
        <?endforeach;?>
                ...
                <div class="panel_content" id="infonews_content" style="display: block;">
                    <table style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <td class="_value">
                                <input type="text" value="<?php if($rs->modified!=0){echo date('H:i:s d-m-Y',$rs->modified);}else{echo date('H:i:s d-m-Y',time());}?>">    
                            </td>
                        </tr>
                    </table>
                </div>
