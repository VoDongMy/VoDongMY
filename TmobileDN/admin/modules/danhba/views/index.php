...
<!--        <td>
        ...
        <td>
            <select onchange="window.open(this.value,'_self');" name="cat_id" id="cat_id" style="margin-bottom: 10px;float: right;width: 200px;">
                <option value="<?=site_url($url.'?catid=0'.$key_url)?>">Xem tất cả danh mục</option>
                <?foreach($listcategory as $val):
                $listcat1 = $this->danhba->get_all_category($val->cat_id);
                ?>
                ...
                $listcat2 = $this->danhba->get_all_category($val1->cat_id);
                ?>
...
<table class="admindata">
    ...
    $k=1;
    foreach($list as $rs):
    $cat = $this->danhba->get_cat_by_id($rs->cat_id,1);
    ?>
    <tr class="row<?=$k?>">
